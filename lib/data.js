...
// Current first-photo resource name for a place. Cheap, single-field Place
// Details call — used by the photo proxy to recover when a snapshot's cached
// photo name has expired (Google rotates them; they must not be cached
// long-term). Returns null when the place genuinely has no photos.
export async function fetchPhotoName(placeId, googleKey) {
  const res = await fetch(`${PLACES_BASE}/places/${placeId}`, {
